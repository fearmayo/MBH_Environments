...
def extract_galaxy_properties(
    ...
    # Load sinks (to classify PartType5)
    ...
    for G in galaxies:
        ...
        # ------------------------------
        # BH remnant mass (from sinks, not snapshot)
        # ------------------------------
        ...
        # ------------------------------
        # Stellar component:
        #   PartType4 = inactive PopII stars  (always stars)
        #   PartType5 = BH + PopIII + active PopII → classify w/ sinks
        # ------------------------------
        ...
        # Combine stars from both types
        star_pos  = np.vstack([star_pos4] + ([np.array(star_pos5)] if len(star_pos5)>0 else []))
    ...
    # ------------------------------------------------------------
    # Dump the 5 most massive BH hosts
    # ------------------------------------------------------------

    # Sort galaxies by BH remnant mass (descending)
    sorted_gals = sorted(
        results.values(),
    ...
    print("\n================ TOP 5 MOST MASSIVE BH REMNANTS ================\n")
    for g in top5:
